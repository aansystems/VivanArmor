...
$query_one = "SELECT a.`course_id` FROM `blocked_courses` AS a, `courses_assigned` AS b WHERE a.`course_id` = b.`courses_assigned` AND a.`user_id` = b.`user_id` AND a.`user_id` ='" . $model->user_id . "'";
...
if ($final_blocked_ids != "") {
    ...
    $query->select(['courses.id', 'courses.course_name'])
            ...
            ->where(['courses_assigned.user_id' => $model->user_id]);
                    ...
                    <div class="card-content">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <h4 class="card-title">LEARNERS DETAILS</h4>

                                <table  class="table table-striped table-bordered detail-view">
                                    <tbody>
                                        <tr><th>LEARNER NAME</th><td><?php echo $user->first_name ?></td></tr>
                                ...
                                <?php if ($role_type != 3) { ?> 
                                    ...
                                    if ($final_blocked_ids != "") {
                                        ...
                                        $query->select(['courses.id', 'courses.course_name'])
                                                ...
                                                ->where(['courses_assigned.user_id' => $model->user_id, 'courses_assigned.blocked_status' => 1]);
                                        $command = $query->createCommand();
                                        $filtered_courses = $command->queryAll();
                                        ?>
                                        <?=
